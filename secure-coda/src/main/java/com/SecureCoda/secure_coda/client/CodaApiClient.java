...
public interface CodaApiClient {
    ...
    //Added path "/docs/{docId}" inside the annotation
    @DeleteMapping("/docs/{docId}")
    void deleteDoc(@RequestHeader("Authorization") String token,
                   @PathVariable("docId") String docId);

    @GetMapping("/docs/{docId}/tables")
    CodaTableResponse listTables(@RequestHeader("Authorization") String token,
                                 @PathVariable("docId") String docId);

    @GetMapping("/docs/{docId}/tables/{tableId}/rows")
